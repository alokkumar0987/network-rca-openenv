...
Use this checklist before submitting your Hugging Face Space URL.

- [ ] `openenv validate` passes locally without errors.
- [ ] HF Space is deployed and responds with HTTP 200.
- [ ] `/reset`, `/step`, `/state` endpoints work end-to-end.
- [ ] `/tasks` returns task list and action schema.
- [ ] `/grader` returns deterministic score in `[0.0, 1.0]` with breakdown.
- [ ] `/baseline` runs successfully and returns reproducible results.
- [ ] Docker image builds successfully from scratch (`docker build -t network-rca-env .`).
- [ ] Docker container starts and serves API on port `7860`.
- [ ] At least 3 tasks exist across easy/medium/hard with grader support.
- [ ] Baseline script runs with fixed seed and produces stable scores.
- [ ] `README` includes setup, action/observation spaces, tasks, rewards, endpoints, and baseline usage.


Acknowledgements:

This environment was built for the Meta Hackathon by team 3xNinja
...
- The user must run `extract_tasks.py` on their own database to generate `data/tasks.json` with real incidents. For testing, they can keep the placeholder (which will cause fallback to minimal tasks, but those are not realistic). They should run the extraction script before final submission.
- All endpoints (`/reset`, `/step`, `/state`, `/baseline`, `/grader`, `/tasks`) are implemented.
- The environment passes `openenv validate` (assuming correct setup).
- The Dockerfile builds and runs the server on port 7860.
- The baseline script is reproducible.

This is a **top‑tier** submission that fully meets the hackathon criteria.
